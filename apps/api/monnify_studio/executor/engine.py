...
MVP behaviour:
  ...
  * The walk runs inside a traced(`executor.run`) span so API request spans
    nest the interpreter (D15).
"""
...
def run_workflow(
    ...
    *,
    ...
    # One OTel span wraps the full walk so API -> executor -> adapter nest (D15).
    with traced(
        "executor.run",
        workflow_id=workflow.id,
        adapter=adapter.name,
        ...
        try:
            ...
            while queue:
                ...
                if result.waiting:
                    ...
                    if not auto_resume_waits:
                        span.set_attribute("status", RunStatus.WAITING.value)
            ...
            span.set_attribute("status", RunStatus.COMPLETED.value)
            span.set_attribute("nodes", len(seen))
