...
def payroll() -> Workflow:
    ...
    nodes = [
        ...
        _n("bulk", "monnify.bulk_transfer", "Bulk Salary Transfer", 720, 0),
        _n("qstatus", "monnify.query_transfer_status", "Query Batch Status", 960, 0),
        _n("recon", "safety.reconciliation", "Reconcile Payouts", 1200, 0),
        _n("audit", "safety.audit_event", "Audit Trail", 1440, 0),
    ]
    edges = [
        Edge(source="payday", target="rows", kind="event"),
        Edge(source="rows", target="validate"),
        Edge(source="validate", target="bulk"),
